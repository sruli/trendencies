...
def get_colors():
    return ["rgb(16, 32, 77)","rgb(160, 32, 77)","rgb(160, 32, 77)","rgb(160, 32, 77)","rgb(160, 32, 77)"]


def get_trendency_size(trendency):
    return random.randint(5, 10)


def combine_key_word_reporter(reporter, key_word):
    return str(reporter) + "-" + key_word


def bubble_trendencies_plot(reporters):
    colors = get_colors()
    data = []
    color_index = 0
    for reporter in reporters:
        # print("reporter! ", reporter)
        for key_word_and_trendencie in reporter.stories_trendencies:
            # print("key_word_and_trendencie: ", key_word_and_trendencie)

            key_word = key_word_and_trendencie[0]
            trendencie = key_word_and_trendencie[1]
            trendencie_size = get_trendency_size(trendencie)
            key_word_and_reporter = combine_key_word_reporter(reporter, key_word)
            place = get_space(key_word)

            trace = get_trace(key_word_and_reporter, trendencie_size, place, colors[color_index])
            # print("trace: ", trace)
            data.append(trace)
        color_index += 1

    # print("data-bubble: ", data)

    layout = go.Layout(title="Industrial Bubble Demo") # here we could insert the reporters names
    fig = go.Figure(data=data, layout=layout)
    plot_div = plot(fig, output_type='div', include_plotlyjs=False)
    return plot_div


def get_trace(key_word_and_reporter, size, place, color="rgb(16, 32, 77)"):
    x = [place[0]]
    y = [place[1]]
...
def bubble_demo():
    ...
    data = [trace0, trace1]
    layout = go.Layout(title="Bubble demo")
    fig = go.Figure(data=data, layout=layout)
    plot_div = plot(fig, output_type='div', include_plotlyjs=False)
    return plot_div


def get_line_trace(rep_story):
    print("rep-story: ", rep_story)
    rep = rep_story[0]
    story = rep_story[1]
    x_list = rep.stories_coverage[story][0]
    y_list = rep.stories_coverage[story][1]
    trace = go.Scatter(
        x=x_list,
        y=y_list,
        mode = 'lines+markers',
...
def line_table(info):
    data = [get_line_trace(info[i]) for i in range(len(info))]
    print("data-type: ", type(data))

    layout = go.Layout(title="Bibi demo")
    #data = [trace]
    fig = go.Figure(data=data, layout=layout)
    plot_div = plot(fig, output_type='div', include_plotlyjs=False)
    return plot_div
...
def my_plot2():
    ...
    layout = go.Layout(title="simple example")
